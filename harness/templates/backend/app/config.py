...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            log_dir=_env_path("APP_LOG_DIR", var_dir / "logs"),
            engine=os.environ.get("APP_ENGINE", "auto").lower(),
            vllm_host=os.environ.get("APP_VLLM_HOST", "127.0.0.1"),
            vllm_port=_env_int("APP_VLLM_PORT", 8000),
            vllm_command=os.environ.get("APP_VLLM_COMMAND", ""),
            vllm_venv=_env_path("APP_VLLM_VENV", PROJECT_ROOT / ".venv-vllm"),
            # Cold starts also compile/fetch kernels, not just load weights.
            startup_timeout=_env_float("APP_STARTUP_TIMEOUT", 1800.0),
            shutdown_timeout=_env_float("APP_SHUTDOWN_TIMEOUT", 30.0),
