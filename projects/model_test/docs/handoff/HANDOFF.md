...
| 등록 모델 | 5종(`models/README.md`) — 실서빙 확인은 TASK-003/004 검증 문서 참조 |
| 테스트 | 백엔드 37건, 프론트 13건 통과 |
...
5. **프론트는 빌드가 없다.** 파일을 고치고 브라우저를 강력 새로고침(Ctrl+Shift+R)하면 끝이다. 캐시된 예전 JS가 남아 증상이 재현되는 경우가 있었다.
...
- 답변 비교는 파라미터 차이와 나란히 보기까지만 제공한다. 외부 심판 모델을 쓰는 자동 채점은 범위 밖이다(SPEC 5).
