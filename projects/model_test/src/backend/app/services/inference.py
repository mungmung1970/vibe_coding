"""Turns a UI prompt request into an engine call and a stream of UI events."""
...
logger = logging.getLogger("model_test.inference")
...
class InferenceService:
    ...
    def generate(self, request: dict) -> Iterator[dict]:
        ...
        model = self.manager.require_ready(request.get("model_id"))
        ...
        run = {
            ...
            "engine": self.manager.engine.name,
            "prompt": prompt,
            "system_prompt": system_prompt,
            "parameters": resolved.values,
            "images": [image["name"] for image in images],  # 기록에는 파일명만 남긴다
        }
        return self._stream(run, payload, resolved)

    def _validate_images(self, model, images: Any) -> list[dict]:
    ...
    def _stream(self, run: dict, payload: dict, resolved: ResolvedParameters) -> Iterator[dict]:
        ...
        try:
            for chunk in self.manager.engine.chat_stream(payload):
