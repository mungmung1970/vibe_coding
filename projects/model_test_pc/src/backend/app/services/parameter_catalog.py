...
class ParameterCatalog:
    ...
    def specs_for(self, model: Model | None = None) -> list[dict]:
        ...
        for raw in document["parameters"]:
            ...
            if providers and profile and profile not in providers:
                continue
            # 모델이 개별적으로 거부하는 항목도 감춘다. 동작하지 않는 컨트롤을
            # 보여주면 값을 바꿔도 반영되지 않아 오해를 부른다.
            if model is not None and spec["key"] in model.unsupported_fields:
                continue
