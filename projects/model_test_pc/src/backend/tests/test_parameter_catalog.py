...
class ParameterCatalogTest(unittest.TestCase):
    ...
    def test_schema_hides_reasoning_parameters_for_plain_models(self) -> None:
        ...
        with_reasoning = self.catalog.schema_for(self.registry.get("endpoint-llm"))
        without = self.catalog.schema_for(self.registry.get("vision-api"))
    ...
    def test_model_overrides_change_defaults_and_bounds(self) -> None:
        schema = self.catalog.schema_for(self.registry.get("endpoint-llm"))
    ...
    def test_resolve_splits_values_into_engine_buckets(self) -> None:
        model = self.registry.get("endpoint-llm")
    ...
    def test_resolve_fills_defaults_and_omits_null_seed(self) -> None:
        resolved = self.catalog.resolve(self.registry.get("vision-api"), {})
    ...
    def test_gated_parameter_is_dropped_when_its_gate_is_off(self) -> None:
        resolved = self.catalog.resolve(self.registry.get("vision-api"), {"logprobs": False, "top_logprobs": 5})

        self.assertNotIn("top_logprobs", resolved.body)

    def test_out_of_range_and_unknown_values_are_rejected(self) -> None:
        model = self.registry.get("vision-api")
    ...
    def test_provider_support_hides_unsupported_parameters(self) -> None:
        ...
        local = {spec["key"] for spec in self.catalog.specs_for(self.registry.get("endpoint-llm"))}
        self.assertIn("repetition_penalty", local)

        remote = Model(id="r", name="R", provider="OpenAI", description="",
                       provider_type="openai", base_url="http://x/v1", remote_model="r", capabilities=("chat",))
    ...
    def test_empty_list_values_are_not_sent(self) -> None:
        resolved = self.catalog.resolve(self.registry.get("endpoint-llm"), {"stop": []})

        self.assertEqual(resolved.values["stop"], [])
        self.assertNotIn("stop", resolved.body)

    def test_model_specific_unsupported_fields_are_hidden(self) -> None:
        # 모델이 거부하는 항목은 요청에서 빠질 뿐 아니라 화면에도 나오면 안 된다.
        model = self.registry.get("vision-api")  # unsupported_fields = ["stop"]
        keys = {spec["key"] for spec in self.catalog.specs_for(model)}

        self.assertNotIn("stop", keys)
        self.assertIn("max_tokens", keys)

    def test_hidden_fields_are_also_rejected_as_input(self) -> None:
        with self.assertRaises(ApiError) as caught:
            self.catalog.resolve(self.registry.get("vision-api"), {"stop": ["END"]})
