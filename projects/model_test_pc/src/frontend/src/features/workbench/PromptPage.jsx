...
export default function PromptPage() {
  const {
    selectedModel, answer, parameters, images, prompt, systemPrompt,
    setPrompt, clearAnswer, submitPrompt, cancelPrompt, saveAnswer, addImages, removeImage, setNotice,
  } = useWorkbench();
  const [raw, setRaw] = useState(false);
  const fileInput = useRef(null);

  const isVlm = selectedModel?.modality === 'VLM';
      ...
      <section className="panel input-panel">
        ...
        <div className="panel-footer">
          ...
          <span className="footer-right">
            {answer.streaming && <button type="button" className="mini-btn" onClick={cancelPrompt}>중단</button>}
          </span>
        </div>
      </section>
